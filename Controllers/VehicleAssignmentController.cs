...
namespace PersonaXFleet.Controllers
{
    ...
    public class VehicleAssignmentController : ControllerBase
    {
        ...
        [HttpPost("Assign")]
        public async Task<IActionResult> AssignVehicleToUser([FromBody] AssignVehicleDto assignDto)
        {
            ...
            try
            {
                ...
                var approvedRequest = await _context.VehicleAssignmentRequests
                    .FirstOrDefaultAsync(r => r.UserId == assignDto.UserId
                                              && r.Status == VehicleRequestStatus.Approved
                                              && r.CurrentStage == "Complete");

                if (approvedRequest == null)
                {
                    return BadRequest("No approved vehicle request found for this user");
                }
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, $"Error assigning vehicle: {ex.Message}");
            }
        }
            ...
            if (isRequestorInCurrentStage)
            {
                ...
                if (completedUsers.Count >= requiredUsers.Count)
                {
                    ...
                    if (nextStage == "Complete")
                    {
                        ...
                        return Ok(new
                        {
                            Message = "Vehicle request approved and completed",
                            RequestId = request.Id
                        });
                    }
        ...
        public async Task<IActionResult> GetMyPendingVehicleActions([FromQuery] string userId)
        {
            ...
            var requests = await (from ur in _context.UserRouteRoles
                                  ...
                                  join r in _context.VehicleAssignmentRequests.Include(r => r.User)
